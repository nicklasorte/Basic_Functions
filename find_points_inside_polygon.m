function [inside_idx]=find_points_inside_polygon(app,poly_contour,points_latlon)

%%%%%%%%Functionalize: Find points inside polyshape
inside_idx=NaN(1,1);
%tic;
contour_latlon=fliplr(poly_contour.Vertices);
...
if ~isempty(temp_contour)
    ...
    if ~isempty(check_latlon_idx)
        ...
        for pos_idx=1:1:num_lat_check
            tf_inside=isinterior(poly_contour,points_latlon(check_latlon_idx(pos_idx),2),points_latlon(check_latlon_idx(pos_idx),1));
